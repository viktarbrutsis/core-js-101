...
function doRectanglesOverlap(rect1, rect2) {
  ...
  return b1 > rect2.top && b2 > rect2.left;
}
...
function isInsideCircle(circle, point) {
  const side1 = point.x - circle.center.x;
  const side2 = point.y - circle.center.y;
  if (Math.sqrt(side1 ** 2 + side2 ** 2) < circle.radius) {
    return true;
  } return false;
}
...
function getDigitalRoot(num) {
  const result = num;
  while (result > 9) {
    result.toString().split().reduce((a, b) => a + b);
  }
  return result;
}
